...
RUN git apply --check /tmp/shockolate-sdl-renderer-fallback.patch \
    ...
    && grep -R "ADLMIDI_EMU_DOSBOX\|Requested SDL_mixer audio\|Opened SDL_mixer audio\|Mix_SetPostMix\|snd_output_rate\|SDL_PauseAudioDevice" -n src/MusicSrc/MusicDevice.c src/MacSrc/SDLSound.c src/MacSrc/Xmi.c src/GameSrc/cutsloop.c src/GameSrc/audiolog.c
